...
export async function fetchCardData() {
  try {
    ...
    return {
      numberOfInvoices: Number(data[0].total_invoices || 0),
      numberOfCustomers: Number(data[0].total_customers || 0),
      totalPaidInvoices: formatCurrency(data[0].total_paid || 0),
      totalPendingInvoices: formatCurrency(data[0].total_pending || 0),
    };
  } catch (error) {
    console.error('❌ Database Error:', error);
